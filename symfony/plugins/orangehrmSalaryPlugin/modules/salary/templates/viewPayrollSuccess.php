...
//                $query = "SELECT p.date_added,e.emp_firstname AS firstname,e.emp_lastname AS lastname, SUM(s.basic + s.transport + s.house_rent + s.medical + s.utilities) AS allowance, SUM(d.eobi+d.provident_fund+d.advance_or_loan+d.wh_tax) AS deduction FROM hs_hr_payroll p INNER JOIN hs_hr_emp_salarybreak_allowance s ON s.id IN (198,197,199,200) INNER JOIN hs_hr_emp_salarybreak_deduction d ON s.deduction_id = d.id INNER JOIN hs_hr_employee e ON p.perform_by = e.emp_number WHERE EXTRACT(MONTH FROM p.date_added) = '07'";
                $query = "SELECT p.date_added,p.id,e.emp_firstname AS firstname,e.emp_lastname AS lastname,p.status, SUM(s.basic + s.transport + s.house_rent + s.medical + s.utilities) AS allowance, SUM(d.eobi+d.provident_fund+d.advance_or_loan+d.wh_tax) AS deduction FROM hs_hr_payroll p INNER JOIN hs_hr_emp_salarybreak_allowance s ON s.id IN (p.salary_id) INNER JOIN hs_hr_emp_salarybreak_deduction d ON s.deduction_id = d.id INNER JOIN hs_hr_employee e ON p.perform_by = e.emp_number WHERE p.id <> 0";
